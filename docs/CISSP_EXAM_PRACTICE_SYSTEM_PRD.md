...
| FR-PAPER-03 | 题库页：学员浏览已发布试卷列表（名称、题量、时长、总分、domain 徽章），可查看试卷详情与自己的作答状态 | P0 |
| FR-PAPER-04 | 一键开练：`POST /api/papers/{id}/sessions`，`mode=practice` 创建练习会话（逐题即时判定与解析）、`mode=exam` 创建考试会话（计时、答题卡、交卷后统一评分） | P0 |
| FR-PAPER-05 | 试卷答题页参照参考样式：左侧/顶部信息区（时长、满分、已用时间、模式切换、答题卡四态统计、自动保存指示、交卷按钮），主区逐题作答（题型徽章、纠错/标记/收藏、双语切换） | P0 |
...
| FR-PAPER-12 | 断点续作：练习/考试中途退出后可重入——题库页「进行中」列表（`GET /api/papers/sessions/in-progress`）提供继续入口；答题页重入经 `GET /api/papers/sessions/{id}/state` 恢复答题卡状态（已答/答错位号）、跳转首个未答题目、恢复问答题已存文本（delivery 的 `previous_answer` 携带 `text`）与已用/剩余时间；练习模式用时以服务端心跳累计（`POST /api/practice/sessions/{id}/heartbeat`，客户端约 20s 上报一次，离开期间不计时，单次上报增量有上限防跳变）；考试模式沿用服务端截止时间 | P0 |
...
GET    /api/papers/{paper_id}/sessions        # 我在该试卷的历史会话
GET    /api/banks                             # 自由练习题库（数据集级入口，FR-PAPER-10）
GET    /api/papers/sessions/in-progress       # 我进行中的试卷/题库会话（断点续作入口，FR-PAPER-12）
...
39. 练习/考试中途退出（刷新、关页、经导航离开）后，从题库页「进行中」重入：答题卡已答/答错状态、当前进度、问答题已存文本、已用时间（练习）或剩余倒计时（考试）全部恢复；练习模式离开期间不计时（FR-PAPER-12）。
