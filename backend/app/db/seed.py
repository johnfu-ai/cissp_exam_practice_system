...
def run_seed(session: Session) -> dict:
    ...
    # mock papers paper dataset (PRD v1.4 FR-PAPER / FR-ETL-17). Questions
    # carry domain_number directly (papers 一..八 -> domains 1..8), so no
    # ChapterDomainMapping rows are needed. Registered only when the dataset
    # directory exists (it is generated by app.etl.paper_export and committed).
    mock-paper_path = "docs/questions/mockpapers"
    if Path(mock-paper_path).is_dir():
        _get_or_create(
            session,
            EtlDataset,
            slug="mockpapers",
            defaults={
                "organization_id": personal_org.id,
                "name": "CISSP 模拟试卷",
                "source_path": mock-paper_path,
                "format": ImportFormat.json,
                "total_questions": 828,
                "languages": ["en", "zh"],
                "notes": "8 mock-paper papers, 828 bilingual single-choice questions",
            },
        )
        session.flush()
        counts["datasets"] = 2
