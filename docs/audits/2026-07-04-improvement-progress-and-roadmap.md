...
All five Tier 2 items shipped on `feat/p2-tier2-prod-readiness` (#25 backups+Redis durability, #24 workers+graceful shutdown, #26 observability, #27 hardened Dockerfiles, #28 compose split+limits+restart) and **merged to `master`** (`fb868a1`, 2026-07-26). See `docs/superpowers/specs/2026-07-20-tier2-prod-readiness-design.md` for the design. 567 backend tests (+5 observability), both images build multi-stage + non-root, full-stack e2e-verified (`docker compose up` -> migrate -> /health, /metrics, X-Request-ID, login, frontend, backup dump, graceful SIGTERM shutdown). Remaining backlog: frontend #32 (OpenAPI types), #35 (product decision), #23-coverage, P2/P3 polish. Tier 3 #21 + #22 closed (`fix/p1-test-infra`, 570 backend tests).
...
| 22 | Real downgrade + drift-meta tests | ✅ DONE (`fix/p1-test-infra`) - `test_upgrade_then_downgrade_succeeds` now asserts the DB is at head + drift-free after a base↔head round-trip; new `test_drift_guard_actually_detects_a_change` is a test-of-the-test (a throwaway probe table must be surfaced by `compare_metadata`); new `test_question_migration_downgrade.py` pins the lossy `a1b2c3d4e5f6` downgrade contract (only `en` restored onto `questions.stem`/`explanations`; option content NOT unpacked; zh permanently lost on round-trip). |
| 23 | Concurrency + N+1 query-count tests; coverage | ◐ PARTIAL - query-count tests done (`c5bdb7d`); CAT-race safety covered by the row-lock+unique-constraint work (`0b22e44`); **no `pytest-cov`/coverage threshold** remains open. | 1–2 days |
| 21 | Make conftest not require CREATEDB | ✅ DONE (`fix/p1-test-infra`) - `_role_can_create_db()` probes `pg_roles.rolcreatedb`; the session `engine` fixture drops/creates the test DB when CREATEDB is available, else falls back to `_reset_schema()` (drop_all+create_all) on a pre-created test DB with a clear actionable error if it's missing. Unblocks the in-container/non-superuser test-runner. 570 backend tests, zero drift. |
