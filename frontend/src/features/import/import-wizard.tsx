...
function Count({ label, value, tone }: { label: string; value: number; tone?: "create" | "update" | "muted" | "error" }) {
  const color =
    ...
    tone === "update" ? "text-sky-600" :
    tone === "error" ? "text-rose-600" : "text-foreground";
  return (
...
export function ImportWizard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="mb-3 text-sm font-medium text-muted-foreground">Datasets</h3>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            {summary.errors.length > 0 && (
              <div>
                <h4 className="mb-2 text-sm font-medium text-rose-600">
            ...
            {run.phase === "preview" ? (
              <div className="flex gap-2">
                <Button onClick={doCommit} disabled={commit.isPending}>
                  {commit.isPending ? "Committing…" : `Commit import (${summary.would_create + summary.would_update} rows)`}
                </Button>
                <Button variant="outline" onClick={doRollback} disabled={rollback.isPending}>
                  Discard
                </Button>
              </div>
            ) : (
