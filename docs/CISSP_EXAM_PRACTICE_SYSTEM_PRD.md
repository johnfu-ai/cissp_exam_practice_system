...
> - v1.4 (2026-09-25)：产品向「参考（mock-paper）风格试卷练习」转型。移除 Flutter 学员端，改为微信小程序学员端 + Web 学员端（Next.js 同时承载学员与管理）。新增：试卷（Paper）与题库浏览（FR-PAPER，§6.14）、问答题题型与参考答案自评（FR-ESSAY，§6.15）、持久化错题集（FR-WRONG，§6.16）、参考导出数据适配（FR-ETL-17..19）、测试与 e2e 非功能需求（NFR-TEST，§7.6）。同步更新 §6.4、§7.4、§8、§9、§10、§12、§13、§14、§15、§17、§18。
...
| FR-ETL-17 | 参考导出适配：提供转换器将 `docs/paper_exam_exports/` 的参考试卷导出（`paper_*.json` + `wrong_questions_*.json`）转换为标准数据集目录（§10.3），含中英双语拆分（行内混合文本按首个中文边界切分 en/zh）、选项字母前缀剥离、逐项解析合并、试卷元数据抽取 | P0 |
...
试卷（Paper）是对「一套固定顺序、固定分值的题目集合」的建模，来源为参考导出等外部试卷或后续人工组卷。题库页面以试卷为入口组织练习，替代单纯按知识点随机抽题作为主路径（随机练习仍保留，见 §6.5）。
...
| FR-PAPER-03 | 题库页：学员浏览已发布试卷列表（参考样式）——统计行（试卷总数/已完成/错题数）+ 试卷卡片（名称、时长、总分、题量、最高得分、已考 N 次/尚未参加状态徽章、开始练习/开始考试/做题记录按钮） | P0 |
...
| FR-PAPER-09 | 参考错题历史导入：将 `wrong_questions_*.json` 的个人错题记录（错 N 次、未掌握）预填到指定用户的错题集 | P1 |
...
| FR-PAPER-12 | 断点续作（v1.6 修订）：题库页**不再**设「进行中」独立区块；点击开始练习/开始考试时，若该试卷+模式存在进行中会话（依据 `GET /api/papers/sessions/in-progress` 客户端过滤），弹出「继续答题」确认框（文案：检测到您有未完成的答题记录，是否继续上次的答题？）——「继续上次答题」直达原会话，「重新开始」新建会话；答题页重入经 `GET /api/papers/sessions/{id}/state` 恢复答题卡状态（已答/答错位号）、跳转首个未答题目、恢复问答题已存文本（delivery 的 `previous_answer` 携带 `text`）与已用/剩余时间；练习模式用时以服务端心跳累计（`POST /api/practice/sessions/{id}/heartbeat`，客户端约 20s 上报一次，离开期间不计时，单次上报增量有上限防跳变）；考试模式沿用服务端截止时间 | P0 |
...
| FR-PAPER-14 | 做题记录（v1.6 新增）：题库页试卷卡片「做题记录」按钮打开弹窗——顶部统计（考试次数/最高分/最低分/平均分，按考试会话计）+ 历史明细表（考试时间/用时/得分/正确/总题数/状态（及格/不及格/进行中/练习）/操作「详情」）；数据源 `GET /api/papers/{id}/sessions`，响应项扩展 `answered`、`score`、`max_score`、`passed`、`duration_seconds`；「详情」跳转考试报告页 | P0 |
...
错题集是跨会话持久化的个人错题本（v1.4 前错题仅从答题记录临时推导），以参考错题页为交互基准。
...
| ID | 需求 |
|---|---|
| NFR-TEST-01 | 后端以真实 PostgreSQL 的 pytest 集成测试为主（含迁移无漂移守卫）；新增功能（试卷、问答题、错题集、参考导入）必须测试先行（TDD） |
...
POST   /api/practice/sessions/{session_id}/heartbeat # {elapsed_seconds} 练习用时心跳（FR-PAPER-12）
...
- **`app/etl/paper_export.py`**：参考导出转换器（FR-ETL-17）——读取 `docs/paper_exam_exports/` 的 `paper_*.json`/`wrong_questions_*.json`，产出标准数据集目录（`manifest.json` + `questions.jsonl` + `papers.json`）；纯函数、可单测、CLI 可执行（`python -m app.etl.paper_export <exports_dir> <out_dir>`）。
...
| question_ids | 题目外部 ID 有序数组（作答顺序） | `["paper-63-...","..."]` |
...
12. **参考双语拆分（v1.4，转换器规则）**：题干按 `<p>` 块或行内首个中文边界拆分 en/zh（仅中文时 en 为空、仅英文时 zh 为空，`available_languages` 相应收敛）；选项剥离 `A.` 字母前缀后同规则拆分；`answerAnalysis` 中非空项合并为逐项解析，正确项解析优先作为题目解析；Word 导出残留标签（`<p class="MsoNormal">`、`&nbsp;`、`font-family` span）归一化剥离；解析中的外链图片（`<img src="https://...">`）保留 URL、按白名单清洗规则处理。
...
4. 试卷与题库（FR-PAPER-01..07、10..13）：参考导出转换 + ETL 幂等导入 8 份试卷、828 道双语单选题（含逐项解析）；数据集级自由练习入口（OSG v10 恢复可达）、练习⇄考试模式切换、断点续作、答题卡稳定性与滚动。
...
12. ETL 管道：以 `docs/questions/` 数据集为输入（OSG v10 + 参考 ），完成抽取、清洗、幂等加载。
...
| Phase W1 | 1-2 周 | 后端：问答题、试卷、错题集模型与 API（TDD）、参考转换器 + ETL 扩展、8 份试卷导入 |
...
29. 参考转换器可将 `docs/paper_exam_exports/` 全部 8 份试卷转换为标准数据集：828 道题、题干与选项双语拆分、选项字母前缀剥离、正确项与逐项解析保留；ETL 预览-确认后幂等入库为 8 份已发布试卷（FR-ETL-17/18）。
...
41. 题库页对齐参考样式：统计行（试卷总数/已完成/错题数）、试卷卡片（时长/总分/最高得分/已考 N 次/尚未参加状态、开始练习/开始考试/做题记录）；做题记录弹窗展示考试次数/最高分/最低分/平均分与历史明细（考试时间/用时/得分/正确/总题数/状态/详情）（FR-PAPER-03/14）。
...
| 参考导出数据质量（行内混合双语、Word 残留标签、外链图片） | 拆分错漏、富文本不可控 | 转换器纯函数化 + 快照测试覆盖全部 828 题；白名单清洗；图片按 URL 保留并清洗 |
...
2. 参考数据先行：转换器 + 数据集 + ETL 导入是全部学员端功能的前置。
...
| 试卷（Paper） | 一套固定顺序、固定分值的题目集合，来源为参考导出或人工组卷（FR-PAPER） |
| 问答题（essay） | 自由文本作答 + 分语言参考答案 + 学员自评判分的题型（FR-ESSAY） |
| 错题集 | 跨会话持久化的个人错题本，含错题/收藏/纠错三个页签（FR-WRONG） |
| 参考（mock-paper） | 参考天下 CISSP 培训班（）配套考试系统的导出数据，本系统首批试卷来源 |
