...
def run_seed(session: Session) -> dict:
    ...
    # Mock paper dataset (PRD v1.4 FR-PAPER / FR-ETL-17). Questions
    ...
    papers_path = "docs/questions/mockpapers"
    if Path(papers_path).is_dir():
        _get_or_create(
            ...
            defaults={
                "organization_id": personal_org.id,
                "name": "CISSP 模拟试卷",
                "source_path": papers_path,
                "format": ImportFormat.json,
                "total_questions": 828,
                "languages": ["en", "zh"],
                "notes": "8 mock papers, 828 bilingual single-choice questions",
            },
        )
        session.flush()
        counts["datasets"] = 2
