...
| 9 | Concurrent-401 refresh race → singleton refresh promise | ◐ PARTIAL | `6558ec7` — singleton done; **tokens still in `sessionStorage`** (`auth-store.ts:34-35`), not httpOnly cookie |
...
| 16 (data) | Read/derive `difficulty` from source in ETL | Engine fixed, but `difficulty=DIFFICULTY_DEFAULT` for every question → ability-matched selection (§11.1 items 1+4, FR-CAT-05) is effectively meaningless. | 1 day |
| 18 | Per-option explanations + read `difficulty`/`license` from source | FR-ANS-04 data model supports per-option explanations but ETL writes `None` (`load.py:157`); `difficulty` and `license_status` hardcoded. | 1 day |
