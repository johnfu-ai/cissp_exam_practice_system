...
- **Frontend**: Next.js 14 (App Router), TypeScript, Tailwind CSS, shadcn/ui (Radix primitives) + `class-variance-authority`, DM Sans via `next/font/google`. Server components by default; only add `'use client'` where interactivity is needed.
...
- `src/components/app-sidebar.tsx` — client component owning nav, the permission-gated "Manage" group, the in-sidebar **language-mode `<Select>`** (en/zh/bilingual, backed by `/api/users/me/preferences`), and logout. Nav items use Apple pill styling (active = `bg-accent text-foreground`).
...
- `src/features/<context>/` — feature modules hold the real page logic (e.g. `features/exam/cat-runner.tsx`, `features/practice/runner-machine.ts`). **Pages in `src/app/.../page.tsx` are often thin wrappers** — when editing a page's behavior or styling, look in the corresponding `features/` module, not just `page.tsx`. Several feature modules have unit tests (`features/**/__tests__/`) that define their contracts (e.g. `cat-runner.test.tsx` asserts the language toggle never advances the item; `editor.test.tsx` asserts FR-LANG-09 publish-completeness validation) — preserve these when restyling.

### Cross-cutting frontend rules

- **Language toggle is pure client state.** In practice/exam runners, switching en/zh/bilingual must only update local `mode` state — it must **never** call `/next` or any advance/submit function (this is the CAT forward-only invariant, enforced by `cat-runner.test.tsx`). Delivery endpoints already return BOTH languages so no refetch is needed.
